...
class TwitterBot:
    ...
    def sync_follows(self, twitter_handle):
        #followers_list = List of Twitter follower IDs
        #follows_list = List of Twitter follows IDs
        ...
        while next_cursor != 0:
            followers_status = self.twitter_connection.followers.ids(screen_name=self.twitter_handle,
                                                               cursor=next_cursor)
            followers = set(followers_status["ids"])
            followers_list.append(followers)
            next_cursor = followers_status["next_cursor"]
        ...
        # SYNC FOLLOWS
        follows_list = []
        following_status = self.twitter_connection.friends.ids(screen_name=self.twitter_handle)
    ...
    def follow_followers(self, count=None):

        not_following_back = self.followers_list - self.follows_list
    ...
    def follow_followers_of_user(self, twitter_handle, count=5000):

        following = self.sync_follows()
